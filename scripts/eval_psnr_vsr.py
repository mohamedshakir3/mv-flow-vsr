...
def list_images(d):
    files = [p for p in glob.glob(os.path.join(d, '*')) if os.path.splitext(p)[1].lower() in IMG_EXTS]
    files.sort()
    return files
...
def psnr_from_arrays(a, b, maxv=255.0):
    # expects uint8 arrays with same shape
    if a.shape != b.shape:
...
def main():
    ...
    ap.add_argument('--mode', default='y', choices=['y','rgb'], help='PSNR on Y channel (with YCrCb) or RGB')
    ap.add_argument('--shave', type=int, default=4, help='border shave in pixels (use scale for x4)')
    ...
    sr_files = list_images(args.sr_dir)
    gt_files = list_images(args.gt_dir)
    ...
    for name in tqdm(names, desc='eval'):
        sr = cv2.imread(sr_map[name], cv2.IMREAD_COLOR)
        gt = cv2.imread(gt_map[name], cv2.IMREAD_COLOR)
        if sr is None or gt is None:
            continue
        sr_rgb = cv2.cvtColor(sr, cv2.COLOR_BGR2RGB)
        gt_rgb = cv2.cvtColor(gt, cv2.COLOR_BGR2RGB)

        if args.mode == 'y':
            sr_y = to_y_channel_rgb(sr_rgb)
            gt_y = to_y_channel_rgb(gt_rgb)
            sr_y = shave_border(sr_y, args.shave)
            gt_y = shave_border(gt_y, args.shave)
            ps = psnr_from_arrays(sr_y, gt_y, maxv=255.0)
        else:
            sr_rgb = shave_border(sr_rgb, args.shave)
            gt_rgb = shave_border(gt_rgb, args.shave)
            ps_r = psnr_from_arrays(sr_rgb[...,0], gt_rgb[...,0])
            ps_g = psnr_from_arrays(sr_rgb[...,1], gt_rgb[...,1])
            ps_b = psnr_from_arrays(sr_rgb[...,2], gt_rgb[...,2])
            ps = (ps_r + ps_g + ps_b) / 3.0

        psnrs.append(ps)
        rows.append((name, ps))

    mean_psnr = float(np.mean(psnrs)) if psnrs else 0.0
    print(f"Frames evaluated: {len(psnrs)} | Mean PSNR ({args.mode}, shave={args.shave}): {mean_psnr:.3f} dB")
    ...
    if args.csv:
        ...
        with open(args.csv, 'w', newline='') as f:
            w = csv.writer(f)
            w.writerow(['frame','psnr_'+args.mode])
            for name, ps in rows:
