...
def encode_vid4_clip(hr_dir, out_mp4_path, scale=4, crf=25):
    """
...
def generate_partition_maps(csv_path, num_frames, height, width, out_dir):
    ...
    try:
        df = pd.read_csv(csv_path); df.columns = df.columns.str.strip()
    except: df = pd.DataFrame(columns=['framenum'])
...
def process_vid4(hr_root, out_root, mv_extractor, crf):
    ...
    print(f"--- Processing Vid4 (CRF {crf}) ---")
    ...
    for clip in CLIPS:
        ...
        clip_out_dir = out_path / f"Vid4_CRF{crf}" / clip
        dirs = {k: clip_out_dir / k for k in ["lr", "mv_fwd", "mv_bwd", "residual", "partition_maps"]}
        for d in dirs.values(): d.mkdir(parents=True, exist_ok=True)

        temp_mp4 = clip_out_dir / f"{clip}.mp4"
        encode_vid4_clip(clip_hr_dir, temp_mp4, scale=4, crf=crf)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hr_root", required=True, help="Path to Vid4 GT folder (containing calendar, city...)")
    ...
    process_vid4(args.hr_root, args.out_root, args.mv_extractor, args.crf)